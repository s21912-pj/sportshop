...
import {Info,addProductToCart} from '../utils/HelperFunctions'
import { Row, Col, Card, Button,Divider } from 'antd'
import ProductType from '../filter/ProductType'
const { Meta } = Card;

const WomanSection = () => {
    var products = global.products.filter(x => x.category === 3);

    const [Products, setProducts] = useState(products)

    const [Filters, setFilters] = useState({
        isEnabled: false,
        productTypeWoman: { isEnabled: false, data: [] },
    ...
    const getAllProducts = (entry) => {
        ...
        if (entry.filters.isEnabled) {
            if (entry.filters.sportwearMaterialType.isEnabled) {
                ...
                setProducts(products)
            } if (entry.filters.productTypeWoman.isEnabled) {
                let filterSet = new Set(entry.filters.productTypeWoman.data)
        ...
        } if (!(entry.filters.sportwearMaterialType.isEnabled && entry.filters.brand.isEnabled && entry.filters.productTypeWoman.isEnabled)) {
            setProducts(products)
        }
    }
    ...
    const handleChangeOnCheckbox = (filters, category) => {
        ...
        switch (category) {
            case 'productTypeWoman':
                newFilters[category].data = filters;
                newFilters.isEnabled = true;
                newFilters.productTypeWoman.isEnabled = filters.length > 0 ? true : false;
                break;
    ...
    return (
        <div style={{ width: '75', margin: '3rem auto' }}>
            ...
            <div style={{ float: 'left', margin: '3rem auto', maxWidth: '160px' }}>
                <ProductType handleProductTypeFilter={filters => handleChangeOnCheckbox(filters, "productTypeWoman")} productTypes={global.product_types_woman} />
