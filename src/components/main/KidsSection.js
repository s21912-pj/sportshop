...
const KidsSection = () => {
    var products = global.products.filter(x=>x.category===4);
    const [Products, setProducts] = useState(products)

    const [Filters, setFilters] = useState({
        isEnabled: false,
        productTypeKid: {isEnabled:false,data:[]},
   ...
   /* useEffect(() => {
       ...
       Filters.isEnabled
            ...
            : setProducts(global.kidsproducts)
    ...
    const getAllProducts = (entry) => {
        ...
        if (entry.filters.isEnabled) {
            if(entry.filters.sportwearMaterialType.isEnabled){
                ...
                setProducts(products)
            }if(entry.filters.productTypeKid.isEnabled){
                let filterSet = new Set(entry.filters.productTypeKid.data)
        ...
        } if(!(entry.filters.sportwearMaterialType.isEnabled&&entry.filters.brand.isEnabled&&entry.filters.productTypeKid.isEnabled)){
            setProducts(products)
        }
    }
    ...
    const handleChangeOnCheckbox = (filters, category) => {
        ...
        switch (category) {
            case 'productTypeKid':
                ...
                newFilters.productTypeKid.isEnabled = filters.length > 0 ? true : false;
                break;
    ...
    return (
        <div style={{ width: '75', margin: '3rem auto' }}>
            ...
            <div style={{ float: 'left', margin: '3rem auto', maxWidth: '160px'}}>
                <ProductType handleProductTypeFilter={filters => handleChangeOnCheckbox(filters, "productTypeKid")} productTypes={global.product_types_kids} />
